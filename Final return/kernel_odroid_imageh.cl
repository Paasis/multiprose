...
write_imageh(output ,coord2 , grey);
}
...
for(j=y;j<=y+window;j=j+1){
	for(i=x;i<=x+window;i=i+1){
			std1=std1+(read_imageh(greyscale1,sampler,(int2){i,j}).x-mean1)*(read_imageh(greyscale1,sampler, (int2) {i,i}).x-mean1);
...
for(d=0;d<=ndisp;d=d+1){
	...
	for(j=y;j<=y+window;j=j+1){
		for(i=x;i<=x+window;i=i+1){
            ...
            mean2=mean2+read_imageh(greyscale2,sampler,(int2){i-d,j}).x;
		}
	}
	...
	for(j=y;j<=y+window;j=j+1){
		for(i=x;i<=x+window;i=i+1){
            ...
            std2=std2+(read_imageh(greyscale2,sampler, (int2) {i-d,j}).x-mean2)*(read_imageh(greyscale2,sampler, (int2) {i-d,j}).x-mean2);
             
			//z value            
            z=z+((read_imageh(greyscale1,sampler, (int2) {i,j}).x-mean1)*(read_imageh(greyscale2,sampler, (int2) {i-d,j}).x-mean2));
...
write_imageh(disp,coord2,best_d);  
}
								...
								__write_only image2d_t processed)
{
	...
	if(abs(read_imageui(disp_left, sampler, coord).x - read_imageui(disp_right, sampler, (int2){x - read_imageui(disp_left, sampler, coord).x, y}).x) > threshold)
	{
		write_imageh(processed, coord, 0);
	}
	else
	{
		write_imageh(processed, coord, read_imageh(disp_left, sampler, coord).x);
	}
}
						...
						__write_only image2d_t final_image)
{
	...
	if(read_imageh(processed, sampler, coord).x != 0){
		write_imageh(final_image, coord, 3.938*read_imageh(processed, sampler, coord).x);
	}
	else
	{
		for(i = 1; i < searchrange; i=i+1){
			if (read_imageh(processed, sampler, (int2){x, y-1}).x != 0){
				write_imageh(final_image, coord, 3.938*read_imageh(processed, sampler, (int2){x, y-1}).x);
				break;
			}	
			else if (read_imageh(processed, sampler, (int2){x-i, y}).x != 0){
				write_imageh(final_image, coord, 3.938*read_imageh(processed, sampler, (int2){x-i, y}).x);
				break;
			}	
			else if (read_imageh(processed, sampler, (int2){x+i, y}).x != 0){
				write_imageh(final_image, coord, 3.938*read_imageh(processed, sampler, (int2){x+i, y}).x);
			...
			else if (read_imageh(processed, sampler, (int2){x-i, y+i}).x != 0){
				write_imageh(final_image, coord, 3.938*read_imageh(processed, sampler, (int2){x, y+i}).x);
				break;
			}	
		}
	}
